...
export default class BobRoss {
    ...
    init() {
        this.#_el.addEventListener('click', function() {
            if (this.#_el.classList.contains('ipsum--flip')) afficheFace(this.#_el, this.#_elImg);
     ...
     */
    afficheDos() {
        let randomIndex = Math.floor(Math.random() * this.#_citations.length);
            
        this.#_elCitation.innerHTML = `<p>${this.#_citations[randomIndex]}</p>
